    ...
    for freq=[1 3]
        ...
        for Nen=1:3
            ...
            if (Nen==1 || Nen==2) && Nm==3  && freq==1
                plot(log10(movmean(squeeze((mean(error_Schur(Nm,freq,Nen,1:2,:),4))),100)),'LineWidth',2)
            else
                plot(log10(movmean(squeeze((mean(error_Schur(Nm,freq,Nen,:,:),4))),100)),'LineWidth',2)
            end
            hold on

            plot(log10(movmean(squeeze((mean(error_Ledoid(Nm,freq,Nen,:,:),4))),100)),'LineWidth',2)
