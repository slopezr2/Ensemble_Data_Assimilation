...
%===Outer Loop===    
 ...
 %==Inner Loop==
     ...
     for k=2:inner
         ...
         d=y(:,i)-H*xk(:,k);
         norma(k,i)=norm(rho(1,k)*Bsquare*alpha(:,k));
     end
     meanxa(:,i)=real(xk(:,end));
%==End Inner Loop==
 for Nen=1:N
     alpha_a=alpha(:,end)+(eye(n,1)+Q'*inR*Q)^(1/2)*randn(n,1);
     xa(:,i,Nen)=real(meanxa(:,i)+rho(1,k)*Bsquare*alpha_a)-0.1+0.2*rand(n,1);
 end
 process=i/tsim*100
end
...
plot(Xreal(1,1:1000))
hold on
plot(meanxa(1,1:1000))
 figure
plot(Xreal(15,1:1000))
hold on
plot(meanxa(15,1:1000))
 figure
plot(Xreal(25,1:1000))
hold on
plot(meanxa(25,1:1000))
 figure
plot(Xreal(35,1:1000))
hold on
plot(meanxa(35,1:1000))
 figure
imagesc(Xreal(:,1:1000)),caxis([-10 10]),colorbar
figure
imagesc(meanxa(:,1:1000)),caxis([-10 10]),colorbar