        ...
        N=Ensem(en); % Number of ensemble
            for rad=1:3
                ...
                for gamma=1:5
                    ...
                    inner=30; % Max Number of inner iterations
                    tolerance_inner=0.2;
                    ...
                    for exp=1:20
                        ...
                        %===Ensemble generation===
                        ensemble_sparse=10;
                        x0=ensemble_sparse/2 -ensemble_sparse*rand(n,N); %The ensemble is generated by the initial condition
                        xb(:,1,:)=x0;
                        xa=xb;
                        meanxb=mean(xb(:,1,:),3);
                        ...
                        %===Outer Loop===    
                         ...
                         if sum(muestreo==i)   %Validation if there are a observation available
                             ...
                             %==Inner Loop==
                                 ...
                                 flaq=0;
                                  while norma(k-1,i)>=tolerance_inner && k<inner && flaq<2 
                                     ...
                                     try alpha(:,k)= pinv((eye(n,1)+Q'*inR*Q))*(Q'*inR*d);
                                     catch
                                         N_errores(obs,freq,en,rad,gamma,exp)=N_errores(obs,freq,en,rad,gamma,exp)+1;
                                         k=k+1;
                                         continue
                            ...
                            %=Analysis ensemble calculation=
                             for Nen=1:N
                                 try alpha_a=(alpha(:,k-1)+real(pinv(eye(n,1)+Q'*inR*Q)^(1/2))*randn(n,1));
                                 catch    
                                    N_errores(obs,freq,en,rad,gamma,exp)=N_errores(obs,freq,en,rad,gamma,exp)+1;
                                    continue
                                  end
                                ...
                                norma_error(obs,freq,en,rad,gamma,exp,muestra)=norm(y(:,i)-yxa(:,i),2);
                             else
                                 xa(:,i,:)=xb(:,i,:);
                                 meanxa(:,i)=meanxb;
                                 yxa(:,i)=operador(salidas,meanxa(:,i),tipo);
