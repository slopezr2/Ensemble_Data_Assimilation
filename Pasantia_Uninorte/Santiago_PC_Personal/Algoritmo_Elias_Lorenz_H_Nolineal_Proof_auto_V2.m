                         ...
                         if sum(muestreo==i)   %Validation if there are a observation available
                             ...
                             %==Inner Loop==
                                 ...
                                 flaq=0;
                                  while norma(k-1,i)>=tolerance_inner && k<inner && flaq<2 
                                     ...
                                     try alpha(:,k)= pinv((eye(n,1)+Q'*inR*Q))*(Q'*inR*d)-sum(alpha(:,1:k-1),2);
                                     catch
                                         N_errores(obs,freq,en,rad,gamma,exp)=N_errores(obs,freq,en,rad,gamma,exp)+1;
                                         k=k+1;
                                         continue
                                     ...
                                     if flaq==2
                                        alpha(:,k)=alpha(:,k-1); 
                                        xk(:,k)=xk(:,k-1);axis([0 25 10^-3.5 10^4])
                                     end
                                     k=k+1;
                                 end
                                 meanxa(:,i)=(xk(:,k-1));
                            ...
                            %=Analysis ensemble calculation=
                             for Nen=1:N
                                ...
                                norma_error(obs,freq,en,rad,gamma,exp,muestra)=norm(y(:,i)-yxa(:,i)./y(:,i),2);
                             else
                                 xa(:,i,:)=xb(:,i,:);
                                 meanxa(:,i)=meanxb;
                                 yxa(:,i)=operador(salidas,meanxa(:,i),tipo);
