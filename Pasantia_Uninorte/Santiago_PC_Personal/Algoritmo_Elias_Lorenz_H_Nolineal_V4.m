...
%===Outer Loop===    
 ...
 Xb=Xb*sqrt(1/(N-1));   
 XB_time(:,:,i)=Xb; 
 [B,Bsquare]=Calculo_B_Cholesky(Xb,r); %Estiaxis([0 25 10^-3.5 10^4])mation of Covariance by Modified Cholesky
 ...
 %==Inner Loop==
     ...
     flaq=0;
      while norma(k-1,i)>=tolerance_inner && k<inner && flaq<2 
         ...
         alpha(:,k)= pinv((eye(n,1)+Q'*inR*Q),0.01)*(Q'*inR*d)-sum(alpha(:,1:k-1),2);
         %=Calculate of rho=
...
%=Analysis ensemble calculation=
 for Nen=1:N
     alpha_a=(alpha(:,k-1)+real(pinv(eye(n,1)+Q'*inR*Q,0.01)^(1/2))*randn(n,1));
%      alpha_a=add_inflation*(real(pinv(eye(n,1)+Q'*inR*Q,0.01)^(1/2))*randn(n,1));
     jjj(:,i,Nen)=(rho*Bsquare*alpha_a);
     xa(:,i,Nen)=(meanxa(:,i)+rho*Bsquare*alpha_a)-add_inflation/2+add_inflation*rand(n,1);
 end
  norma_error_rela(:,i)=norm((yxa(:,i)-y(:,i))./y(:,i));
  norma_error_muestras(:,muestra)=norm((y(:,i)-yxa(:,i))./y(:,i),2);
...
% figure
% semilogy(norma_error_muestras), title('Norm Relative Error')
figure
semilogy(norma_error_muestras)
% figure
